...
(defn- generate-rows-and-fields
  ...
  (let [
       ...
       rows (if (and sort-opt (some #{sort-opt} (conj fields true)))
              (sort-by
               #(nth % (if (true? sort-opt) 0 (.indexOf ^java.util.List fields sort-opt)))
               rows) rows)
...
(defn- render-rows-with-fields [rows fields options]
  (let [
    ...
    widths (czlab.table.width/get-widths (cons headers rows))
    fmt-row (fn [row]
              (map-indexed
...
(defn format-cell [^String string width]
  (if (zero? width)
    ""
    (format
      (str "%-" width "." width "s")
